...
clad의 중간 다리. 짧게 살다 죽는 CLI와 tmux pane 안에서 도는 Claude를 **하나의 영구 프로세스**가 이어줍니다. 첫 `clad` 호출이 알아서 띄우고, 셸을 닫아도 살아남고, idle 시간이 지나면 알아서 정리합니다.

소스: [`src/clad/bridge/`](src/clad/bridge/)

### 왜 필요한가

- `claude -p` 한 번 호출은 컨텍스트가 누적되지 않음 → 매번 새 세션이 시작됨.
- 그렇다고 사용자가 `claude`를 직접 띄워두면 → 프롬프트 주입, 출력 캡처, 동시 구독 같은 걸 직접 짜야 함.
- 그래서 가운데에 작은 라우터(데몬)를 둠. **CLI는 HTTP만, Claude는 MCP 도구만** 알면 됨.
...
사용자 입력
    │
    ▼
clad CLI ──HTTP POST /sessions/{key}/prompt──► 데몬 (큐에 적재)
                                                  │
                                                  │ Claude가 long-poll로 풀어감
                                                  ▼
                                          Claude (tmux pane)
                                                  │
                                                  │ 토큰/완료 신호를 MCP 도구로 보냄
                                                  ▼
                                                데몬 (SSE로 fanout)
                                                  │
                       ◄──── HTTP GET /sessions/{key}/stream ────
clad CLI (사용자 터미널에 토큰 스트리밍)
```
...
| 일 | 한 줄 설명 |
|---|---|
| 자동 기동 | CLI가 `~/.clad/bridge.pid`/`bridge.port`를 보고 없으면 `python -m clad.bridge`를 detached로 띄움 |
| 세션 관리 | `(프로젝트, 태그)`별로 tmux pane을 만들고, Claude를 띄우고, MCP 설정을 주입 |
| 프롬프트 큐 | 사용자 입력을 받아 큐에 넣음. Claude는 `clad_get_prompt` 도구로 풀음 |
| SSE 스트리밍 | Claude가 내보낸 토큰을 구독 중인 모든 CLI에 동시 전달 (fanout) |
| 재연결 복구 | 최근 1000개 이벤트를 버퍼링. 끊긴 클라이언트가 `Last-Event-ID`로 다시 붙으면 빠진 부분만 replay |
| Idle 자동종료 | `idle_timeout_minutes` 동안 활동 없는 세션은 정리. `--keepalive` 세션은 예외 |
| Config hot-reload | `~/.clad/config.yaml` 수정 시 데몬 재시작 없이 즉시 적용 |

### MCP 도구 3개

데몬이 Claude pane에게 노출하는 도구 — 이게 "채널"입니다:

| 도구 | 역할 |
|---|---|
| `clad_get_prompt` | 큐에서 다음 프롬프트를 꺼냄 (없으면 최대 30초 대기) |
| `clad_emit_token(text)` | 응답 토큰 한 조각을 사용자 터미널에 푸시 |
| `clad_emit_done(summary?)` | 응답 완료 신호 |

콜드스타트 직후 데몬이 pane에 한 줄짜리 부트스트랩 지시를 입력합니다:
...
이걸 받은 Claude는 무한 루프로 큐를 풀고, 사용자 프롬프트는 적재되는 즉시 전달됩니다.

### 직접 띄우고 만져보기

```sh
# 전경 실행 (stdout 로그를 보면서 디버깅)
...
전체 HTTP/MCP 컨트랙트와 모듈 시그니처는 [`AGENTS.md`](AGENTS.md)에 정리되어 있습니다.
